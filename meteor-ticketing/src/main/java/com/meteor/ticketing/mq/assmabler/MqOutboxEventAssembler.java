...
public class MqOutboxEventAssembler {
    ...
    /**
     * 构建TicketDbReserved事件
     */
    public MqOutboxEvent buildTicketDbReserved(String orderNo,
                                               String payload,
                                               LocalDateTime deliverAt,
                                               LocalDateTime bizExpireAt) {
        return new MqOutboxEvent()
                .setId(idGenerator.nextId())
                .setBizKey(orderNo)
                .setEventType(EVT_TICKET_DB_RESERVED)
                .setExchangeName(TicketOrderContract.Exchange.TICKET_ORDER)
                .setRoutingKey(TicketOrderContract.RoutingKey.TICKET_ORDER_CREATE)
                .setPayload(payload)
                .setStatus(OutboxStatus.NEW)
     ...
     * 通用构建方法
     */
    public MqOutboxEvent build(String bizKey,
                               String eventType,
                               String exchangeName,
                               String routingKey,
                               String payload,
